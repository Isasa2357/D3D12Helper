...
D3D12ProcessingConstants MakeConstants(
    ...
    if (srcRect.width != dstRect.width || srcRect.height != dstRect.height) {
        throw ValidationError(std::string(functionName) +
            ": format conversion does not resize; use D3D12Resizer or D3D12FusedProcessor");
    }
    ...
    if (IsYuv420Format(dstFormat)) {
        ValidateYuv420Rect(dstRect, functionName, "dstRect");
    }
...
void D3D12FormatConverter::RecordConvert(
    ...
    RecordConvertImpl(
        ...
        desc,
        state,
        "D3D12FormatConverter::RecordConvert");
}

void D3D12FormatConverter::RecordConvertView(
    D3D12CommandContext& commandContext,
    D3D12ResourceView src,
    D3D12ResourceView dst,
    const FormatConvertDesc& desc,
...
void D3D12FormatConverter::RecordConvertImpl(
    D3D12CommandContext& commandContext,
    D3D12ResourceView src,
    D3D12ResourceView dst,
    D3D12Resource* trackedSrc,
    ...
    if (!IsSupportedProcessingFormat(srcFormat) || !IsSupportedProcessingFormat(dstFormat)) {
        throw UnsupportedFormatError(std::string(functionName) + ": unsupported format");
    }
    ...
    if (IsRgbaLikeFormat(srcFormat) && IsRgbaLikeFormat(dstFormat)) {
        ...
        RecordYuv420ToRgbImpl(
            commandContext, src, dst, trackedSrc, trackedDst, desc, state, functionName);
    } else if (IsRgbaLikeFormat(srcFormat) && IsYuv420Format(dstFormat)) {
        RecordRgbToYuv420Impl(
            commandContext, src, dst, trackedSrc, trackedDst, desc, state, functionName);
    } else {
        throw UnsupportedFormatError(std::string(functionName) + ": unsupported conversion matrix");
    }
}
...
void D3D12FormatConverter::RecordRgbToRgbImpl(
    D3D12CommandContext& commandContext,
    D3D12ResourceView src,
    D3D12ResourceView dst,
    D3D12Resource* trackedSrc,
    ...
    auto constants = MakeConstants(src, dst, desc, functionName);
    ...
    if (!IsRgbaLikeFormat(srcFormat) || !IsRgbaLikeFormat(dstFormat)) {
        throw UnsupportedFormatError(std::string(functionName) + ": expected RGBA-like formats");
    }

    auto srcViews = CreateRgbaTextureViewSetFromView(*m_context, src, true, false, srcFormat);
    auto dstViews = CreateRgbaTextureViewSetFromView(*m_context, dst, false, true, dstFormat);
    ...
    commandContext.GetCommandList()->SetComputeRootDescriptorTable(
        m_pipelines->rgbToRgb.SrvTableIndex(), srcViews.Gpu(srcViews.srvIndex));
    commandContext.GetCommandList()->SetComputeRootDescriptorTable(
        m_pipelines->rgbToRgb.UavTableIndex(), dstViews.Gpu(dstViews.uavIndex));
    SetRootConstants(m_pipelines->rgbToRgb, commandContext, constants);
    m_pipelines->rgbToRgb.Dispatch(
        commandContext,
...
void D3D12FormatConverter::RecordYuv420ToRgbImpl(
    D3D12CommandContext& commandContext,
    D3D12ResourceView src,
    D3D12ResourceView dst,
    D3D12Resource* trackedSrc,
    ...
    if (!IsYuv420Format(static_cast<DXGI_FORMAT>(constants.srcFormat)) ||
        ...
        throw UnsupportedFormatError(std::string(functionName) +
            ": expected YUV420 -> RGBA-like conversion");
    }
    ...
    commandContext.GetCommandList()->SetComputeRootDescriptorTable(
        m_pipelines->yuv420ToRgb.SrvTableIndex(), srcViews.Gpu(srcViews.ySrvIndex));
    commandContext.GetCommandList()->SetComputeRootDescriptorTable(
        m_pipelines->yuv420ToRgb.UavTableIndex(), dstViews.Gpu(dstViews.uavIndex));
    SetRootConstants(m_pipelines->yuv420ToRgb, commandContext, constants);
    m_pipelines->yuv420ToRgb.Dispatch(
        commandContext,
        DivideRoundUp(constants.dstWidth, kThreadGroupX),
        DivideRoundUp(constants.dstHeight, kThreadGroupY),
        1);
...
void D3D12FormatConverter::RecordRgbToYuv420Impl(
    D3D12CommandContext& commandContext,
    D3D12ResourceView src,
    D3D12ResourceView dst,
    D3D12Resource* trackedSrc,
    ...
    if (!IsRgbaLikeFormat(static_cast<DXGI_FORMAT>(constants.srcFormat)) ||
        ...
        throw UnsupportedFormatError(std::string(functionName) +
            ": expected RGBA-like -> YUV420 conversion");
    }
    ValidateEvenSize(
        constants.dstWidth,
        constants.dstHeight,
        static_cast<DXGI_FORMAT>(constants.dstFormat),
        functionName);

    auto srcViews = CreateRgbaTextureViewSetFromView(
        *m_context, src, true, false, static_cast<DXGI_FORMAT>(constants.srcFormat));
    auto dstViews = CreateYuv420UavViewSetFromView(*m_context, dst);
    ...
    commandContext.GetCommandList()->SetComputeRootDescriptorTable(
        m_pipelines->rgbToYuv420.SrvTableIndex(), srcViews.Gpu(srcViews.srvIndex));
    commandContext.GetCommandList()->SetComputeRootDescriptorTable(
        m_pipelines->rgbToYuv420.UavTableIndex(), dstViews.Gpu(dstViews.yUavIndex));
    SetRootConstants(m_pipelines->rgbToYuv420, commandContext, constants);
    m_pipelines->rgbToYuv420.Dispatch(
        commandContext,
        DivideRoundUp(constants.dstWidth, kThreadGroupX),
        DivideRoundUp(constants.dstHeight, kThreadGroupY),
        1);
