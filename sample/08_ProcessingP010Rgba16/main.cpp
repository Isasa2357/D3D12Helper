...
std::filesystem::path ProcessingShaderDir() {
    ...
    if (std::filesystem::exists(runtimeDir / "ConvertNv12ToRgb.hlsl")) {
        return runtimeDir;
    }
    ...
    if (std::filesystem::exists(sourceDir / "ConvertNv12ToRgb.hlsl")) {
        return sourceDir;
    }
#endif
    return runtimeDir;
...
D3D12Resource CreateUploadedP010(D3D12Core& core, UINT width, UINT height) {
    std::vector<uint16_t> y(static_cast<size_t>(width) * height);
    std::vector<uint16_t> uv(static_cast<size_t>(width) * (height / 2));
    for (UINT py = 0; py < height; ++py) {
        for (UINT px = 0; px < width; ++px) {
            y[static_cast<size_t>(py) * width + px] = static_cast<uint16_t>((px + py) * 8192u);
        }
    }
    for (auto& v : uv) v = 32768;

    auto tex = CreateTexture2D(core, width, height, DXGI_FORMAT_P010, D3D12_RESOURCE_STATE_COPY_DEST);
    ...
    subresources[0].rowPitch = static_cast<UINT64>(width) * sizeof(uint16_t);
    subresources[1].data = uv.data();
    subresources[1].rowPitch = static_cast<UINT64>(width) * sizeof(uint16_t);
...
int main() {
    try {
        ...
        if (!processing.SupportsP010Srv() || !processing.SupportsRgba16FloatUav()) {
            std::cout << "Required P010 SRV or RGBA16F UAV support is unavailable on this device.\n";
            return 0;
        }

        auto src = CreateUploadedP010(*core, 4, 4);

        D3D12FormatConverter converter;
        converter.Initialize(processing);
        auto dst = converter.CreateOutputTexture(*core, 4, 4, DXGI_FORMAT_R16G16B16A16_FLOAT);

        FormatConvertDesc desc = {};
        desc.srcFormat = DXGI_FORMAT_P010;
        desc.dstFormat = DXGI_FORMAT_R16G16B16A16_FLOAT;
        ...
        std::cout << "P010 -> R16G16B16A16_FLOAT conversion completed.\n";
        return 0;
    } catch (const std::exception& e) {
        std::cerr << "D3D12Sample_08_ProcessingP010Rgba16 failed: " << e.what() << "\n";
        return 1;
    }
}
