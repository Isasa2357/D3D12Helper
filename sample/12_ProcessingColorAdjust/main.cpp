...
int main() {
    try {
        ...
        auto dst = adjuster.CreateOutputTexture(
            *core,
            width,
            height,
        ...
        std::cout << "D3D12 color adjust completed. Output: "
                  << dst.GetWidth() << "x" << dst.GetHeight() << "\n";
        return 0;
    } catch (const std::exception& e) {
        std::cerr << "D3D12Sample_12_ProcessingColorAdjust failed: " << e.what() << "\n";
        return 1;
    }
}
