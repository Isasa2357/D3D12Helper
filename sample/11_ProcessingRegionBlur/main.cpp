...
int main() {
    try {
        ...
        ctx.Reset();
        processor.RecordRegionBlur(ctx, src, scratch, blurred, dst, desc);
        ExecuteAndWait(*core, ctx);

        std::cout << "Region blur completed. Output: "
                  << dst.GetWidth() << "x" << dst.GetHeight() << "\n";
        return 0;
    } catch (const std::exception& e) {
        std::cerr << "D3D12Sample_11_ProcessingRegionBlur failed: " << e.what() << "\n";
        return 1;
    }
}
