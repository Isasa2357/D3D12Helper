...
デバイス生成、DXGI アダプタ選択、Queue / Fence / CommandList 管理、リソース作成、Descriptor 管理、Upload / Readback、Compute / Graphics パイプライン、シェーダコンパイル、SwapChain 作成、共有リソース作成、GPU 画像処理（format conversion / resize / remap / composite / fused pass）など、D3D12 アプリケーションで繰り返し書くコードを軽量クラスと自由関数にまとめています。
...
- **D3D12Processing**  
  GPU 上で format conversion、resize、remap、composite、fused convert+resize を実行する Layer 3。NV12 / P010 / RGBA8 / BGRA8 / RGBA16F を扱う。
- **ThrowIfFailed**  
...
|      D3D12FormatConverter / D3D12Resizer                     |
|      D3D12Remapper / D3D12Compositor                         |
|      D3D12FusedProcessor                                     |
...
|      CreateBufferSrv / CreateBufferUav / CreateConstantBufferView |
|      CreateTexture2DRtv / CreateTexture2DDsv                 |
|      RecordUploadTexture2D / RecordUploadTextureSubresources |
+--------------------------------------------------------------+
                    |
                    v
+--------------------------------------------------------------+
...
`D3D12Core` は、D3D12 のデバイス・キュー・フェンス・コマンド記録口を束ねるファサードです。アプリケーション内では 1 つの `std::shared_ptr<D3D12Core>` を作成し、Renderer、Camera、Processing など複数のサブシステムで共有することを想定しています。

Descriptor Allocator や Upload Ring などのフレームごと・用途ごとの状態を持つオブジェクトは、Layer 2 / Layer 3 側で必要に応じて作成します。
...
│   └── D3D12Processing/            # Layer 3 ヘッダ
├── shaders/D3D12Processing/        # Processing Layer HLSL / hlsli
├── src/                            # ライブラリ本体の実装 (.cpp)
├── sample/                         # サンプルコード
│   ├── 01_HelloDevice/
│   ├── 02_ComputeGrayscale/
...
ライブラリ本体は `include/` と `src/` に分離しています。Processing Layer を使う場合は `shaders/D3D12Processing/` も実行時に参照できる場所へ配置してください。
...
int main() {
    try {
        ...
        auto cb = CreateConstantBuffer(*core, 128);     // 256 byte alignment internally
...
| `07_ProcessingFusedConvertResize` | NV12 → RGBA resize を fused pass で実行し、CPU readback で検証 | 不要 |
...
| `D3D12ComputePipeline` | `D3D11ComputePipeline` | Compute Shader 実行補助。D3D12 側も `Bind` / `Dispatch` を持つ |
| `D3D12GraphicsPipeline` | `D3D11GraphicsPipeline` | D3D12 では PSO と Root Signature を含む。D3D12 側も `Bind` を持つ |
| `CreateStructuredBuffer` / `CreateConstantBuffer` / View 作成ヘルパ | D3D11 の用途名ヘルパ | D3D11 と近い名前で D3D12 リソースを作れる |
| `CreateSharedTexture2D` + `D3D12SharedResource` | `CreateSharedTexture2D` | D3D12 では shared heap flag と shared handle が明示的に必要 |
| `D3D12Processing` | 今後の `D3D11Processing` | Processing Layer は D3D11 側でも同等 API を用意する方針 |
