...
- **D3D12Processing**  
  GPU 上で format conversion、resize、remap、composite、blur、region effect、mask、threshold、pyramid blur、affine / perspective transform、3D LUT などを実行する Layer 3。単体 C++ processor はショートカットであり、実務的な連続処理は `shaders/D3D12Processing/*.hlsli` を include した独自 fused HLSL として 1 dispatch にまとめる方針。NV12 / P010 / RGBA8 / BGRA8 / RGBA16F を扱う。
- **ThrowIfFailed**  
...
単体 Processor はショートカットです。実務的な連続処理を 1 dispatch にまとめる場合は、`ProcessingCommon.hlsli` / `ColorSpace.hlsli` などを include した custom fused shader を作ります。例は `sample/18_ProcessingCustomFusedShader` を参照してください。
...
| `18_ProcessingCustomFusedShader` | HLSL library based custom fused Processing sample | 不要 |
